...
![enter description here][3]
 ...
 a：首先加载本地的config.json配置文件，将相关配置读取到dict中等
 ...
 2.1：rewrite_Mod ==> 跳转模块，set-cookie操作
 
 3：app_Mod ==> 用户自定义应用层过滤
 
 4：referer_Mod ==> referer过滤（黑/白名单、log记录）
 
 5：url_Mod ==> url过滤（黑/白名单、log记录）
 
 6：header_Mod ==> header过滤（黑名单）
 
 7：useragent_Mod ==> useragent过滤（黑/白名单、log记录）
 
 8：cookie_Mod ==> cookie过滤（黑/白名单、log记录）
 
 9：args_Mod ==> args参数过滤（黑/白名单、log记录）
 
 10：post_Mod ==> post参数过滤（黑/白名单、log记录）
 
 11：network_Mod ==> 应用层网络频率限制（频率黑名单）
 
 - body阶段
 
 12：replace\_Mod ==> 内容替换规则（动态进行内容替换，性能消耗较高慎用，可以的话用app\_Mod中rehtml、refile这2个自定义action）
