...
![enter description here][7]
 通过图片可以看到，关闭所有规则，做了2组测试，取最高的`8542`，启用规则（排除app，network，replace），测试结果`8388`，性能下降`1.81%`，启用规则（排除replace，app中未启用relua这个高消耗点），测试结果`7959`，性能下降`6.83%`。
 总的来说，启用规则后，性能损失比较小，根据自身的业务进行调整，还可以有所优化。
